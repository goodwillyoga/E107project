...
# Let us normailze volume,closingPrice and priceChange for this duration
df_stocktweetsperday <- df_stocktweetsperday %>% group_by(symbols) %>% mutate_each(funs(normalit), volume) %>% mutate_each(funs(normalit), prcChange) %>% mutate_each(funs(normalit), price) %>% ungroup() 

hist(df_stocktweetsperday$prcChange)
