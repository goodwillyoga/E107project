...
p3<- results_prcChange_count %>% 
  ...
  ggplot( aes(symbol,pval, color=symbol))+geom_point()+
  geom_hline(yintercept = .05, color='red')+ xlab("Symbols") + ylab("Pvalues") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+ ggtitle("Daily Price Change~count model")+ theme(legend.position="bottom")
...
p1 <- results_volume_count %>% 
  ...
  geom_point(size = 2) + ggtitle("Volume~Count model")+ xlab("Symbols") + ylab("Confidence Interval") +
...
p2 <-results_price_count %>% 
  ...
  select(symbol,estimate, pval,`conf.low`, `conf.high`) %>% 
  ggplot( aes(x = symbol, y = estimate, color = symbol)) +
  geom_point(size = 2) + ggtitle("Closing Price~Count model")+ xlab("Symbols") + ylab("Confidence Interval") +
  geom_errorbar(aes(ymax = `conf.high`, ymin = `conf.low`))
...
p3 <-results_prcChange_count %>% 
  ...
  select(symbol,estimate, pval,`conf.low`, `conf.high`) %>% 
  ggplot( aes(x = symbol, y = estimate, color = symbol)) +
  geom_point(size = 2) + ggtitle("Daily Price Change~Count model")+ xlab("Symbols") + ylab("Confidence Interval") +
  geom_errorbar(aes(ymax = `conf.high`, ymin = `conf.low`))
...
p1<- results_volume %>% filter(symbol %in% selectively_analyzed_symbols) %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("Volume~Sentiment Score model")+ theme(legend.position="bottom")

p2 <- results_price %>% filter(symbol %in% selectively_analyzed_symbols)  %>% 
  ggplot( aes(symbol,pval, color=symbol))+geom_point()+ xlab("Symbols") + ylab("P-values") +
  geom_hline(yintercept = .05, color='red')+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+ ggtitle("Closing Price~Sentiment Score model")+ theme(legend.position="bottom")

p3<- results_prcChange %>% filter(symbol %in% selectively_analyzed_symbols)  %>% 
  ggplot( aes(symbol,pval, color=symbol))+geom_point()+ xlab("Symbols") + ylab("P-values")+
  geom_hline(yintercept = .05, color='red')+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+ ggtitle("Price Change~Sentiment Score model")+ theme(legend.position="bottom")
