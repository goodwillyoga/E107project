...
hourlyStockTweetNormalized %>% 
  filter(symbol %in% selectively_analyzed_symbols & day_hr > ymd_hms("2016-04-24 00:00:00")) %>% 
  ggplot()+geom_line(aes(day_hr,avgScore, color="Sentiment Score"))+
  geom_line(aes(day_hr,avgPrice, color="Price")) + facet_wrap(~symbol,scales = "free") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+ theme(legend.position="bottom")
