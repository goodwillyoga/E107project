...
### Methodology for extraction of Data - StockTwits 
  We follwed a similar approach as for Twitter data to collect tweets for the chosen stocks for three weeks from [StockTwits](https://api.stocktwits.com/api/2/streams/symbol/AAPL.json). The dats gathering, cleaning and processing steps can be summarized as below
...
data <- inner_join(dat.plot,ticker_sector)  

#Sector characters are to long
#map.market(id=data$symbol, area=data$price, group=data$sector, color=data$tweet_counts, main="Stock Price and Tweet counts", lab=c(TRUE,TRUE))

data <- dat.plot
map.market(id=data$symbol, area=data$price, group=data$symbol, color=data$tweet_counts, main="Stock Price and Tweet counts", lab=c(FALSE,TRUE))
