...
avg_sentiment_score_by_hour <- data %>% 
            ...
            ungroup() %>%
            select(symbol, dayhour, avg_score, pchange_tweet_counts, pchange_avg_score, scale_pchange_avg_score, scale_pchange_tweet_counts) %>%
            unique() 
...
results <- tidy(fits, mod) %>% filter(term=='scale_pchange_tweet_counts') 

#Plot p-values and see symbols that have significant results
results %>% filter(symbol %in% c("AAPL", "YHOO", "MSFT", "TSLA", "GOOG", "FB", "EIX", "GS", "IBM")) %>%
  ggplot(aes(symbol, p.value, color=symbol)) + geom_point() + geom_hline(yintercept = .05, color = 'red') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("P-values for Normalized percent change in Price ~ Sentiment Score Model")
```
