...
subtitle: "Introduction to Data Science E107 Spring 2016"
author: 
- Mohan Patnam
- Pulkit Bhanot
- Pooja Singh
...
---
_**[Project Github Repository](https://github.com/goodwillyoga/E107project)**_

# Table of contents
1. [Background and Motivation](#Background)
2. [Project Objectives](#Objectives)
3. [Concepts and Methods](#Concepts)
4. [Data Description](#DataDescription)
5. [Data Scraping](#DataScraping)
    1. [Twitter Data Extraction and Processing](#TwitterDataExtraction)
    2. [Yahoo Finance Data Extraction and Processing](#YahooFinanceData)
    3. [StockTwits Data Extraction and Processing](#StockTwitsData)
    4. [Data Normalization](#DataNormalization)
6. [Assumptions](#Assumptions)
7. [Outline](#Outline)
8. [Beta Analysis](#BetaAnalysis)
9. [Beta Computation: Case Study of 3 stocks](#BetaComputation)
    1. [Analysis for a low-beta stock (EIX)](#EIX)
    2. [Analysis for a medium-beta stock (IBM)](#IBM)
    3. [Analysis for a high-beta stock (GS)](#GS)
10. [Regression between beta and stock returns](#RegressionBeta)  
11. [Trend estimation using smoothing](#TrendSmoothing)
12. [Regression between low beta stocks and returns](#RegressionLowBeta)
13. [Analysis of a basket of low-volatility stocks vs high-volatility stocks](#AnalysisBasket)
    1. [MonteCarlo simulation](#MonteCarlo)
14. [Twitter Analysis on Stock volatility](#TwitterAnalysis)  
    1. [Exploratory Data Analysis](#TWTREDA)
    2. [Applying Linear Regression](#TWTRLR)
      1. [Modeling based on count of number of tweets per day](#TWTRCounts)
      2. [Modeling based on Sentiment Score of tweets per day](#TWTRSentiment)
15. [StockTwits Analysis on Stock Volatility](#StockTwits)
    1. [Exploratory Data Analysis](#StockTwitsEDA)
    2. [Applying Linear Regression](#StockTwitsLM)
      1. [Modeling based on count of number of tweets per day](#StockTwitsCounts)
      2. [Modeling based on Sentiment Score of tweets per day](#StockTwitsSentiments)
...
The other important aspect that this research focuses is on the social sentiment and how it can potentially affect Stock's volatility. We primarily wanted to include multiple factors affecting the volatility, just to make this research more useful. Hence the second part of the research focusses on social tweets (such as Twitter and StockTwits) that has been gaining momentum since the advent of Social Media Revolution. We will investigate to see if tweets have any impact on stock volatility on a short term basis, given that the project duration is about one month. We will also run through several regression models to predict if the stock's volatility is affected by factors such as tweet count and sentiment score (as predictors). Finally, we will also ponder a bit about social chatter effect possibly a leading or lagging indicator of stock volatility, if any, though we expect it to be lagging for the most part. 
...
For investing purposes, Beta is further classified into categories (low/medium/high) which helps investors to choose the class of stocks they can invest depending on their risk appetite. Typically, risk-averse investors choose low beta stocks. We will basically be using three categories for our beta analysis in this paper - 0 to 0.80 (Low), 0.80 to 1.30 (Medium) and 1.30+ (High). Note that Beta can be negative as well, which means that stock returns is negatively correlated to the benchmark index returns. However, this is outside the scope of this paper.
...
Also it is clear that returns are different for different beta values. We will later see if there is a clear relation between returns and beta for a universe of 2500 stocks.
...
beta_est<-mutate(beta_est, cor=c(IBM_cor, EIX_cor, GS_cor))
...
The below plot shows the scatterplot of SPY weekly returns plotted against the weekly returns of GS over the same period showing a clear trend. The correlation coefficient (0.72) is very strong suggesting a high correlation with the benchmark (SPY) returns. The beta derived in this case is 1.54 which is typically considered high. This suggests high volatility in stock price compared to the benchmark, presenting a much higher risk for the investors. The R-Squared value came out as 0.52 (Table 4) suggesting that 52% of variation in the GS weekly returns can be explained by the model.

```{r, warning=FALSE, echo=FALSE}
ggplot_lm("GS.Returns")
```

<div id='RegressionBeta'/>
...
3. Assuming Yt represents the expected return on the stock (y-axis) and Xt represents the expected return of the benchmark (S&P500), this model can be generalized as follows - 
$$Y_t = \alpha + X_t*β + \varepsilon_t$$ where β = Beta and α = intercept showing the stock return when there is no risk.

4. Fitting the regression coefficients, we get the equation - 
$$Y_t = 17.16 - 5.48*β + \varepsilon_t$$ where β<=4.5

<div id='TrendSmoothing'/>
...
$$Y_t = 9.83 + 6.44*β$$  where β <= 0.8

<div id='AnalysisBasket'/>
...
dailyStockTweetNormalized %>% 
  ...
  geom_smooth(color='red') +facet_wrap(~symbol)+ xlab("Scaled Tweet Counts") + ylab("Scaled Price")
...
dailyStockTweetNormalized %>% 
  ...
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  geom_smooth(color='red') +facet_wrap(~symbol)+ xlab("Scaled Tweet Counts") + ylab("Scaled Volume")
...
dailyStockTweetNormalized %>% 
  ...
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  geom_smooth(color='red') +facet_wrap(~symbol)+ xlab("Scaled Tweet Counts") + ylab("Scaled Price Change")
...
dailyStockTweetNormalized %>% 
  filter(symbol %in% selectively_analyzed_symbols) %>% 
  mutate(md = lubridate::mday(day)) %>%  
  ggplot()+geom_line(aes(md,volume, color="Volume"))+ xlab("Days in April")+ ylab("Price/Tweet Volume")+
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
...
dailyStockTweetNormalized %>% 
  ...
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  geom_line(aes(md,price, color="Price")) + 
  facet_wrap(~symbol,scales = "free") + theme(legend.position="bottom")
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_tweet_counts,scale_price)) +
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Tweet Counts") + ylab("Scaled Price") +
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_tweet_counts,scale_volume)) +
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Tweet Counts") + ylab("Scaled Volume") +
  geom_point()+geom_smooth(color='red') +facet_wrap(~symbol)
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_tweet_counts,scale_price_change))+
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Tweet Counts") + ylab("Scaled Price Change") +
  geom_point()+geom_smooth(color='red') +facet_wrap(~symbol)
```
...
p1 <-results_price %>% 
  ...
  theme(axis.title.x=element_blank(), 
        axis.ticks=element_blank(),
        axis.title.y=element_blank()) + 
  geom_point(size = 2) + ggtitle("ClosingPrc~Score") +
  geom_errorbar(aes(ymax = `conf.high`, ymin = `conf.low`)) + theme(legend.position="bottom")
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_avg_score,scale_price))+
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Sentiment Score") + ylab("Scaled Price") +
  geom_point()+geom_smooth(color='red') +facet_wrap(~symbol)
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_avg_score,scale_volume))+
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Sentiment Score") + ylab("Scaled Volume") +
  geom_point()+geom_smooth(color='red') +facet_wrap(~symbol)
...
dat.byday %>% filter(symbol %in% selectively_analyzed_symbols) %>% ggplot( aes(scale_avg_score,scale_price_change))+
  theme(axis.title.x = element_text(size = rel(0.75))) +
  theme(axis.title.y = element_text(size = rel(0.75))) +
  xlab("Scaled Sentiment Score") + ylab("Scaled Price Change") +
  geom_point()+geom_smooth(color='red') +facet_wrap(~symbol)
```
...
<div id='Conclusion'/>
# Conclusion 
The detailed statistical analysis is an attempt to present Beta (market factor) and Tweets (social factor) as measures of stock volatility (or risk). The exploratory data analysis of "beta" is shown to be normal market wide as well as with in each sector. Further the case study of 3 distinct stocks (GS, EIX, IBM) demonstrates that the volatility of a stock can be explained to some extent using the volatility of the benchmark index (S&P500) which is what is called as "beta" effect. We demonstrated that utility stocks (like EIX) are less prone to market risks (as expected) as shown by their low beta values. On the other hand, financial stocks (like GS) are most exposed to market risk as shown by their high beta values. The next study of beta using an universe of 2500+ stocks demonstrates that there is a trend between beta and the stock returns. The returns within the universe of low beta stocks are actually better with increasing beta (< 0.8). however, this trend doesn't hold good with stocks having much higher beta (> 1.0) where returns are actually trending lower with increasing beta. This was also confirmed by the Monte Carlo simulation performed by comparing returns of low-beta stocks against high-beta ones.

On social sentiment side, we focussed on both Twitter and StockTwits user feeds. Twitter Data shows that there are more number of tweets for technology sector companies when compared to other sectors. This is partly expected as tech stocks tend to be popular in general. On the other hand, utility stocks tend to have the least chatter in the social media as they are defensive (or rather boring) stocks. We are able to successfully model Daily volumes, Closing prices and Daily Price variations for some stocks like AVP, BSX, XRX, PSX, GILD merely based on tweets volume using Linear Regression. The simple regression model based on sentiment scores with Daily volumes, Closing prices and Daily Price are not good predictors for these models as p-values for majority of the stocks is >.05. However, for the 3 chosen stocks (GS, EIX, IBM) we can see that regression Models using tweets volume can be a good fit for Daily Stock Volume and Price change. Finally on Twitter side, we also see a clear follower relationship between Volume of tweets with Stock Price movements as well as Sentiment scores and Stock price movements. Analyzing trends using Cross correlation may provide more insights into this.

StockTwits data results were slightly different from Twitter results. For example, we got similar results for linear models for GS Daily Volume vs. Tweet counts and we got significant results for IBM for Daily Price vs. Sentiment Score with StockTwits dataset but not with Twitter dataset. This may be attributed to different users contributing to different datasets. In StockTwits data set we did not gather user data so we could not see the overlap of users between the Twitter and StockTwits datasets.

One interesting trend that we found combining both beta and tweets analysis is that, some financial stocks (like GS) showed correlation with both market (beta) and social (twitter) sentiment. In particular, GS returns is noticed to have a high correlation with benchmark returns (S&P500) and further also showed having a modest positive trend with social sentiment score. On the other end of the spectrum, the returns of utility stocks (like EIX) is shielded regardless of benchmark returns (S&P 500) or social media chatter. This is partly expected as utility stocks are defensive stocks. 

Lexicons that we used for sentiments were last modified in the year 2013. We know that slang that people use to express themselves does change. We can modify the lexicon list and add words that are more commonly used now and that might have an impact but we will still see that sentiment scores will always follow the trends seen in Stock Price movements.Lastly we can study the time-lag of the mood/sentiment for better models for market and mood cross-correlation. 
