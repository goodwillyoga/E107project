...
Twitter Data shows that there are more number of tweets for technology sector companies then compared to other sectors. We can successively model Daily volumes, Closing prices and Daily Price variations for some of the stocks like AVP, BSX, XRX, PSX, GILD just based on the number of tweets using Linear Regression. Linear regression model based on sentiment scores with Daily volumes, Closing prices and Daily Price are not good predictors for these models as p-values for majority of the stocks is >.05.  For the 3 chosen stocks we can see that regression Models using Number of tweets can be a good fit for Daily Stock Volume and Price change. 
We see a clear follower relationship between Volume of tweets with Stock Price movements as well as Sentiment scores and Stock price movements. Analyzing the trends using Cross correlation may provide some more insights into this.
...
As the stock markets are closed over the weekend, but people are tweeting continuously, we have some missing data for the weekend stock prices in the model. We can use simple average scheme to fill the missing data for the weekend before fitting the model but is left for future consideration. Lexicons that we used for sentiments were last modified in the year 2013. We know that slang that people use to express themselves does change. We can modify the lexicon list and add words that are more commonly used now and that might have an impact but we will still see that sentiment scores will always follow the trends seen in Stock Price movements.Lastly we can study the time-lag of the mood/sentiment for better models for market and mood cross-correlation. 
